            ...
            <header class="relative z-20 bg-navy text-surface border-b border-surface/10 shadow-md">
                <div class="px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between gap-4">
                    <div class="flex items-center gap-3">
                        ...
                        <a href="{{ route('department.theses.index') }}" class="flex items-center gap-3">
                    ...
                    <div class="flex items-center gap-4">
                        ...
                        @if ($user)
                            <div class="flex items-center gap-2 pl-4 border-l border-surface/10">
                                <span class="grid place-items-center w-9 h-9 rounded-full bg-cyan/20 text-surface text-xs font-bold">
                                    {{ Str::of($user->department?->code ?? $user->name)->substr(0, 2)->upper() }}
                                </span>
                                <span class="hidden sm:block leading-tight">
                                    <span class="block text-sm font-semibold max-w-40 truncate">{{ $user->department?->name ?? $user->name }}</span>
                                    <span class="block text-xs text-surface/60">Department</span>
                                </span>
                            </div>
                        @endif
                    </div>
            ...
            <div class="flex">
                ...
                <aside class="fixed lg:static inset-y-0 left-0 z-10 w-64 shrink-0 bg-sidebar text-surface/80 pt-16 lg:pt-0
                    ...
                    <nav class="p-4 space-y-1">
                        <p class="px-3 pt-2 pb-3 text-xs font-bold uppercase tracking-wider text-surface/40">Department</p>

                        @php
                            $navBase = 'flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-semibold transition';
                            $navOn = 'bg-gold text-navy';
                            $navOff = 'text-surface/80 hover:bg-surface/10 hover:text-surface';
                        @endphp

                        <a href="{{ route('department.theses.index') }}"
                           class="{{ $navBase }} {{ request()->routeIs('department.theses.index') ? $navOn : $navOff }}">
                            <svg class="w-5 h-5" viewBox="0 0 24 24" fill="none" stroke="currentColor"
                                 stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">
                        ...
                        <a href="{{ route('department.theses.create') }}"
                           class="{{ $navBase }} {{ request()->routeIs('department.theses.create') ? $navOn : $navOff }}">
                            <svg class="w-5 h-5" viewBox="0 0 24 24" fill="none" stroke="currentColor"
                                 stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">
                    ...
                    @if ($user)
                        <div class="mt-auto p-4 border-t border-surface/10">
                            ...
                            <p class="text-sm font-bold text-surface truncate">{{ $user->department?->name ?? $user->name }}</p>
                        </div>
                    @endif
                </aside>
